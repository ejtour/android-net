...
public enum GroupParam {
    ...
    P04(4, "只允许合作客户下单", "*开启该功能后，非合作客户查找本店铺商品时，不能查看商品价格，同时也不能提交订单。",
            "只允许合作客户下单？", "开启后，非合作客户不能下单",
            "允许所有客户下单？", "关闭后，非合作客户可以下单"),
    P06(6, "添加我为合作供应商时需要验证", "*不开启需要验证时任意客户都可以与您建立合作关系；反之则需要通过信息验证才能与您建立合作关系。",
            "添加合作供应商需要验证？",
            "添加合作供应商不需验证？"),
    P07(7, "仅接单", "*该功能仅为哗啦啦供应链用户发送采购订单使用，启用该功能后，订单内的商品为采购方增加，供应商无法做商品统计和库存管理！",
            "确认开启仅接单？",
            "确认关闭仅接单？"),
    P08(8, "由供应商填写验货数量", "*启用该功能后验货数量将由供应商(司机)填写，供应商填写后客户根据该数量进行收货结算。当订单中包含已发货未验货的订单时，不可切换验货模式。",
            "开启由供应商验货？", "开启后，验货数量由供应商填写",
            "关闭由供应商验货？", "关闭后，验货数量由客户填写"),
    P09(9, "只允许以协议价进行下单", "*启用该功能后，所有的客户必须有协议报价时，才能查看商品价格和下单。当协议价到期后，如果没有新的协议价，将不能查看商品价格以及下单购买。",
            "开启只允许协议价下单？", "开启后，客户必须由生效协议报价时，才能下单",
            "关闭只允许协议价下单？", "关闭后，客户可在没有协议报价时下单"),
    P10(10, "发货时修改商品价格", "*开启后，供应商发货时，可修改本订单该商品的发货单价或总价。"),
    P11(11, "非营业时间不接单", "*开启停止接单后当超出您的营业时间范围内时，客户不可以向您下单。您可以在店铺信息中修改营业时间。",
    ...
    P15(15, "验货模式", "*开启验货模式，所有订单在验货时都需进行二次确认。",
            "确认开启验货模式？",
            "确认关闭验货模式？"),
    P18(18, "新增合作门店时需要验证", "*不开启需要验证时任意客户门店都可以与您建立合作关系；反之则需要通过信息验证才能与您建立合作关系。",
            "添加合作门店需要验证？",
            "添加合作门店不需要验证？"),
    ...
    P28(28, "仓库发货方式", "开启后可以按照客户所在地区与品类关系拆单。",
            ...
            "确认关闭仓库发货设置开关？", ""),
    P30(30, "报价单OA审核", "*开启报价单OA审核，报价单提交后，需要在企业微信中进行OA审核。",
            "确认开启OA审核？", "开启后，报价单需要进行OA审核",
            "确认关闭OA审核？", "关闭后，报价单自动审核通过"),
    P32(32, "允许客户关联品项", "*启用该功能后，客户可以关联品项。",
            "允许客户关联品项？", "不允许客户关联品项？"),
    P37(37, "支持打印", "*启用该功能后，可以可以进行打印"),
    P38(38,"代客下单支持编辑地址与配送方式","开启后，使用crm进行代客下单时支持编辑地址与配送方式"),
    P39(39,"退货必须传图片","*开启后，收到的退货申请必须包含图片"),
    P40(40, "待结算状态不显示", "*开启该功能后，订单管理中待结算状态不显示",
            "确认开启待结算状态不显示？",
            "确认关闭待结算状态不显示？"),
    ;
    private int type;
