...
@Route(path = RouterConfig.REPORT_WAREHOUSE_DELIVERY)
public class WareHouseDeliveryStatisticsActivity extends BaseLoadActivity implements IWareHouseDeliveryStatisticsContract.IWareHouseDeliveryStatisticsView {

    private static final int COLUMN_NUM = 8;
    private static final int[] WIDTH_ARRAY = {40,170,170, 120, 120, 120, 120, 120};
    ...
    TitleBar mTitleBar;
    private IWareHouseDeliveryStatisticsContract.IWareHouseDeliveryStatisticsPresenter mPresenter;
    WareHouseDeliveryReq mParam = new WareHouseDeliveryReq();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_report_warehouse_delivery_goods_statistics);
    ...
    private void initData() {
        mPresenter = WareHouseDeliveryStatisticsPresenter.newInstance();
    ...
    private void initView() {
        ...
        mExcel.setOnRefreshLoadMoreListener(new OnRefreshLoadMoreListener() {
            ...
            public void onRefresh(@NonNull RefreshLayout refreshLayout) {
                mPresenter.loadWareHouseDeliveryStatisticsList();
            }
        });
    }
    ...
    public void bindEmail() {
        ...
        Utils.bindEmail(this, (email) -> mPresenter.exportDeliveryStatistics(email, reqParams));
    }
    ...
    public void export(String email) {
        Gson gson = new Gson();
        String reqParams = gson.toJson(getRequestParams());
        mPresenter.exportDeliveryStatistics(email, reqParams);
    }


    @Override
    public WareHouseDeliveryReq getRequestParams(){
        return mParam;
    }
    ...
    private View generateHeader(boolean isDisPlay) {
        ...
        if(isDisPlay) {
            ...
            row.updateRowDate("序号", "货主集团", "货主门店", "发货单数", "发货种数", "发货件数", "发货金额", "发货货值");
            row.setBackgroundResource(R.drawable.bg_excel_header);
        }
        return row;
    }

    private View generatorFooter(WareHouseDeliveryResp wareHouseDeliveryResp, boolean isDisplay){
        ExcelRow row = new ExcelRow(this);
        if(isDisplay) {
            row.updateChildView(COLUMN_NUM);
            ExcelRow.ColumnData[] array = new ExcelRow.ColumnData[COLUMN_NUM];
            ...
            array[7] = ExcelRow.ColumnData.createDefaultHeader(UIUtils.dip2px(WIDTH_ARRAY[7]));
            row.updateItemData(array);
            row.updateRowDate("合计", "--", "--", wareHouseDeliveryResp.getTotalDeliveryBillNum()+"",
                    wareHouseDeliveryResp.getTotalDeliveryGoodsSpecNum(), wareHouseDeliveryResp.getTotalDeliveryGoodsNum(),
                    CommonUtils.formatMoney(Double.parseDouble(wareHouseDeliveryResp.getTotalDeliveryAmount())),
                    CommonUtils.formatMoney(Double.parseDouble(wareHouseDeliveryResp.getTotalWareHouseDeliveryGoodsAmount())));
            row.setBackgroundResource(R.drawable.bg_excel_header);
        }
        return row;
    }
