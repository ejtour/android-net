 ...
 * @date 2019/7/20
 */
public class RefundedProductDetailPresenter implements RefundedProductDetailContract.IRefundedProductDetailPresenter {

    private RefundedProductDetailContract.IRefundedProductDetailView mView;
    private int mPageNum;
    private int mTempPageNum;

    static RefundedProductDetailPresenter newInstance() {
        return new RefundedProductDetailPresenter();
    }
    ...
    public void start() {
        ...
        queryRefundedProductDetail(true);
    }



    @Override
    public void register(RefundedProductDetailContract.IRefundedProductDetailView view) {
        this.mView = CommonUtils.checkNotNull(view);
    }

    @Override
    public void queryRefundedProductDetail(boolean showLoading) {
        toQueryRefundedProductDetail(showLoading);
    }

    @Override
    public void loadMoreRefundedProductDetail(){
        mTempPageNum = mPageNum;
        mTempPageNum++;
        toQueryRefundedProductDetail(true);
    }

    private void toQueryRefundedProductDetail(boolean showLoading) {
        RefundedProductReq requestParams = mView.getRequestParams();
    ...
    @Override
    public void exportRefundedProductDetail(String email, String reqParams) {
        if (!TextUtils.isEmpty(email)) {
            bindEmail(email);
            return;
        }
        Report.exportReport(reqParams, "111022", email, new SimpleObserver<ExportResp>(mView) {
            @Override
